...
function updateInquiryTrayUI() {
    ...
    if (!_popoverOpen) {
        ...
        tray.className = 'fixed left-4 right-4 sm:left-auto sm:right-6 z-[100] max-w-[calc(100vw-32px)] w-auto mx-auto sm:mx-0 bg-[#134095] text-white p-2.5 sm:px-4 sm:py-3 rounded-full shadow-2xl border border-white/20 flex items-center justify-between sm:justify-start gap-2.5 transition-all duration-300 transform translate-y-0 opacity-100 overflow-hidden';
        tray.innerHTML = `
            <a href="contact.html#step-3" class="flex items-center gap-3 group text-left min-w-0 flex-1 overflow-hidden">
                ...
                <div class="flex flex-col text-left min-w-0 flex-1 overflow-hidden">
                    <span class="text-[9px] uppercase tracking-widest text-[#EF7F1A] font-bold truncate">Inquiry Tray</span>
                    <span class="text-xs font-semibold truncate text-white/90">${label}</span>
                </div>
            </a>
        ...
        tray.className = 'fixed left-4 right-4 sm:left-auto sm:right-6 z-[100] w-[calc(100vw-32px)] sm:w-80 mx-auto sm:mx-0 bg-[#134095] text-white p-4 rounded-2xl shadow-2xl border border-white/20 flex flex-col gap-3 transition-all duration-300 transform translate-y-0 opacity-100 overflow-hidden';
        tray.innerHTML = `
